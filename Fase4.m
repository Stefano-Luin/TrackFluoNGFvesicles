function [r,h_todo] = Fase4(extr,llls,par,h_todo)
% Copyright (C) 2012 - 2022 Domenica Convertino, Stefano Luin (s.luin@sns.it)
histtype={'v_go_s','v_go_m','v_go_abs_s','v_go_abs_m','vdx','vsn','val_s','val_m','vmed','vmedabs','vmax','vtot'};
...
if nargin<1 || isempty(extr)
    ...
    extr=evalin('caller','extr');
        ...
        warning('Luin:Fase4','Fase4 called with empty or too few arguments');
        r=repmat({[]},size(h_todo(a)));
        return;
    end
end
c=0;
...
if ismember('v_go_abs_s',h_todo)
    c=c+1;
    r{c}=makehist([[extr.v_avsn,[extr.v_avdx]]',[sqrt([extr.v_avsn_s2s]),sqrt([extr.v_avdx_s2s])]',[extr.dtsn,extr.dtdx]'],(-1:0.25:10),[llls par],'v_go_abs_s',1);
end
...
if ismember('v_go_abs_m',h_todo)
    c=c+1;
    r{c}=makehist([[extr.v_avsn,[extr.v_avdx]]',[sqrt([extr.v_avsn_s2m]),sqrt([extr.v_avdx_s2m])]',[extr.dtsn,extr.dtdx]'],(-1:0.25:10),[llls par],'v_go_abs_m',1);
end
...
if ismember('vmedabs',h_todo)
    c=c+1;
    r{c}=makehist([abs([extr.vmed])',[extr.vmed_s]',[extr.t_tot]'],(-1:0.25:10),[llls par],'vmedabs',1);
end
