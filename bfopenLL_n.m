function [result] = bfopenLL_n(id,SerNum,ImgIn,ImgFin, varargin)
...
% Possible changes: 2014 - 2022 Stefano Luin (s.luin@sns.it)
...
%         fprintf('.');
        if ishandle(h1)
            waitbar((i-ImgIn+1)/(ImgFin-ImgIn+1),h1)
        end
        
        if (i-ImgIn+1)==1
            arr = bfGetPlane(r, i, varargin{:});
            imageList = zeros(size(arr,1), size(arr,2), ImgFin-ImgIn+1,'uint16');
        end
...
%         end
        ...
        imageList(:,:,i-ImgIn+1) = bfGetPlane(r, i, varargin{:});
%         imageList{i, 2} = label;
    end
