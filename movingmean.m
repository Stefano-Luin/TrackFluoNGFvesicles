...
% Copyright (C) 2012 - 2022 Carmine di Rienzo (Scuola Normale Superiore, Pisa, Italy)
...
%defaults the dimension and option to first non-singleton dimension or 1 if it is not specified
...
if n(dim)<window
    warning('Luin:movingmean','window is too large')
    if window>2*n(dim)
        window=2*n(dim)-1;
    end
end
